...
bool StealthType::ToText(LSOBJECTDATA ObjectData, char *buf, unsigned int buflen)
{
	...
	strcpy(buf,"ISXStealth");
	return true;
}

bool StealthType::GetMember(LSOBJECTDATA ObjectData, PLSTYPEMEMBER pMember, int argc, char *argv[], LSOBJECT &Object)
{
	...
	switch((StealthTypeMembers)pMember->ID)
	{
	case RetrieveData:
...
bool StealthType::GetMethod(LSOBJECTDATA &ObjectData, PLSTYPEMETHOD pMethod, int argc, char *argv[])
{
	...
	switch((StealthTypeMethods)pMethod->ID)
	{
	case PerformAction:
