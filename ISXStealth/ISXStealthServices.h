#pragma once
/**********************************************************************
 ISXStealthServices.h is a redistributable file that can be used 
 by other extensions to access the services provided by ISXStealth.
...
/* in  (requests) */
#define Stealth_FOO						(ISXSERVICE_MSG+1)
// add all requests

/* out (notifications) */
#define Stealth_BAR						(ISXSERVICE_MSG+2)
// add all notifications

// ----- "Stealth Service" request structures ---------------------
 ...
 * example handles a call to this function:
   bool StealthFooFunction(const char *Name, unsigned int Age,float Height);
...
// ----- "Stealth Service" Helper Functions -----------------------
 ...
 * the remote function.  
 */
static inline bool StealthFoo(ISXInterface *pClient, ISInterface *pISInterface, HISXSERVICE hStealthService, const char *Name, unsigned int Age, float Height)
{
	// set up lpData
	StealthRequest_Foo RemoteFunctionCall;
	...
	return pISInterface->ServiceRequest(pClient,hStealthService,Stealth_FOO,&RemoteFunctionCall) && RemoteFunctionCall.Success;
}
// Most extensions will opt to use the default naming conventions, with a global pISInterface and pExtension,
// and your service handle name.  This means you can make an "easy" macro to call StealthFoo for them,
// and they can just use EzFoo as if they were calling the actual remote function:
#define EzFoo(_name_,_age_,_height_) StealthFoo(pExtension,pISInterface,hStealthService,_name_,_age_,_height_)
// Note that EzFoo is now used exactly the same as bool StealthFooFunction(const char *Name, unsigned int Age,float Height)

// ----- "Stealth Service" notification structures ---------------------
// The following structures are for use in Stealth Service notification handlers
// Stealth_BAR
...
struct StealthNotification_Bar
{
	/* out */ const char *Text;
};
