...
void __cdecl StealthService(ISXInterface *pClient, unsigned int MSG, void *lpData)
{
	switch(MSG)
	{
	case ISXSERVICE_CLIENTADDED:
		...
		// lpData is an ISXInterface* that is the pointer to the new client
		{
			...
			printf("StealthService client added: %X",pNewClient);
	...
	case ISXSERVICE_CLIENTREMOVED:
		...
		// lpData is an ISXInterface* that is the pointer to the removed client
		{
			...
			printf("StealthService client removed: %X",pRemovedClient);
		}
		break;
	case Stealth_FOO:
		// This is a custom service request defined in ISXStealthServices.h
		// pClient is a valid pointer to the client that sent this request
		// lpData is a StealthRequest_Foo* as sent by the client
		{
			StealthRequest_Foo *pFoo=(StealthRequest_Foo*)lpData;

			/*
			 * As described in ISXStealthServices.h, pFoo is simply a remote call
			 * to StealthFooFunction, and has all of the parameters and the outgoing
			 * return value ready to go.
			 */
			pFoo->Success=StealthFooFunction(pFoo->Name,pFoo->Age,pFoo->Height);
 ...
 * How to broadcast an outgoing service message (called a notification):
 	StealthNotification_Bar Bar;
	Bar.Text="Some text to pass as part of the notification";
	pFoo->Success=pISInterface->ServiceBroadcast(pExtension,hStealthService,Stealth_BAR,&Bar);
 */