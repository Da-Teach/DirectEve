...
{
public:
	CTextEditLine(const char *p_Text, unsigned int p_nLine)
	{
		nLine=p_nLine;
		pPrev=0;
		pNext=0;
	...
	bool InsertText(unsigned int Position, const char *Insert, unsigned int InsertSize)
	{
		if (!InsertSize)
			return true;
		if (Position>Used)
	...
	bool InsertCharacter(unsigned int Position, char C)
	{
		if (Position>Used)
			return false;
		if (Used+2>=Allocated)
	...
	bool OverwriteCharacter(unsigned int Position, char C)
	{
		if (Position>Used)
			return false;
	...
	inline bool DeleteCharacter(unsigned int Position)
	{
		if (Position>=Used)
			return false;
	...
	inline bool Truncate(unsigned int Position)
	{
		if (Position>Used)
			return false;
		Text[Position]=0;
...
{
public:
	LGUI_API LGUITextEdit(char *p_Factory, LGUIElement *p_pParent, char *p_Name);
