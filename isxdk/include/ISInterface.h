...
public:
	// paths
	virtual char *GetCWD(char *buffer, unsigned int buflen)=0;
	...
	virtual char *GetArgs(int BeginInclusive, int EndExclusive, char *argv[], char *buf, unsigned int buflen)=0;
	virtual char *GetArgsQuoted(int BeginInclusive, int EndExclusive, char *argv[], char *buf, unsigned int buflen)=0;
	virtual char *GetArgsQuotedNF(int BeginInclusive, int EndExclusive, char *argv[], char *buf, unsigned int buflen)=0;
	virtual void ExecuteTimedCommand(unsigned int DelayMS, const char *Command)=0;
	virtual int ExecuteCommand(const char *Text)=0;
	virtual int DataParse(const char *ToParse, char *buf, int buflen)=0;
	...
	virtual LSTypeDefinition *FindLSTypeDefinition(const char *Name, char *subtypebuf=0, unsigned int subtypebuflen=0)=0;
	...
	virtual bool ResolveCommand(const char *Text, char *buf,unsigned int buflen)=0;
	virtual void *GetTempBuffer(unsigned int Size, void *CopyFrom=0)=0;
	virtual void AddTempObject(CTempObject *pObject)=0;
	virtual bool RunScriptFromBuffer(const char *Name, const char *Buffer, unsigned int buflen, int argc=0, char *argv[]=0, const char *CWD=0)=0;
	...
	virtual void ISLavishScriptInterface_04_Reserved() {}
	virtual void ISLavishScriptInterface_05_Reserved() {}
	...
	virtual unsigned int AddGlobalAtom(const char *Buffer, unsigned int buflen)=0;
	virtual bool RemoveAtom(unsigned int AtomID)=0;
	virtual int ExecuteAtom(unsigned int AtomID,int argc=0, char *argv[]=0, PLSOBJECT pThisLSObject=0, PLSOBJECT pContext=0, char *ReturnBuffer=0, unsigned int buflen=0)=0;
	virtual int ExecuteAtom(const char *Name,int argc=0, char *argv[]=0, PLSOBJECT pContext=0, char *ReturnBuffer=0, unsigned int buflen=0)=0;
	virtual int ExecuteAtom(const char *ScriptName, const char *Name,int argc=0, char *argv[]=0, PLSOBJECT pContext=0, char *ReturnBuffer=0, unsigned int buflen=0)=0;
	...
	// scripts
	virtual bool GetCurrentScript(char *buffer, unsigned int buflen)=0;
	virtual bool GetScriptCWD(const char *ScriptName, char *buffer, unsigned int buflen)=0;
	...
	virtual unsigned int AddAtom(const char *ScriptName, const char *Buffer, unsigned int buflen)=0;
	...
	virtual char *Escape(const char *text, char *buf, int buflen)=0;
	virtual char *EscapeQuotes(const char *text, char *buf, int buflen)=0;
...
public:
	...
	virtual __declspec(deprecated) bool GetSetting(char *Filename, char *Set, char *Setting, char *buf, unsigned int buflen)=0;
	...
	virtual bool GetSetName(unsigned int &SetGUID, char *buf, unsigned int buflen)=0;
	...
	virtual bool GetSetting(unsigned int &SetGUID, const char *Setting, char *buf, unsigned int buflen)=0;
...
public:
	...
	virtual bool GetSessionName(unsigned int nSession, char *buf, int buflen)=0;
...
public:
	...
	virtual bool UnloadExtension(const char *Filename)=0;
	...
	// paths
	virtual char *GetInnerSpacePath(char *buffer, unsigned int buflen)=0;
