...
public:
	...
	virtual unsigned int GetContainerUsed()=0;
...
public:
	...
	virtual bool Clear()
	{
		...
		while(pHead)
		{
			...
			pType->FreeVariable(pHead->Object.ObjectData);
			delete pHead;
			pHead=pNext;
		}
		pTail=0;
	...
	virtual unsigned int GetContainerSize() {return Count;}
	virtual unsigned int GetContainerUsed() {return Count;}
...
public:
	...
	virtual bool Clear()
	{
		...
		while(pTop)
		{
			...
			pType->FreeVariable(pTop->Object.ObjectData);
			delete pTop;
			pTop=pNext;
		}
		return true;
...
public:
	...
	virtual ~LSObjectCollection()
	{
		...
		{
			pType->FreeVariable(i->second->VarPtr);
			delete i->second;
		}
	}
...
/**/
	...
	virtual void SetItem(const char *Key, LSOBJECTDATA &ObjectData)
	{
		...
		{
			// existing
			PLSOBJECTDATA pVal=&i->second->VarPtr;
			pType->FreeVariable(*pVal);

			i->second->VarPtr=ObjectData;
		}
		else
		{
			PLSOBJECT pObject=new LSOBJECT;
			pObject->VarPtr=ObjectData;
			pObject->Type=pType;
			Map[Key]=pObject;
		}
	}
	...
	LSTypeDefinition *GetLSVariableType();
	inline unsigned int GetCount() {return Map.size();}
	inline LSType *GetType() {return pType;}
	inline const char *GetSubType() {return SubType;}
	virtual LSIterator *NewIterator(void *PassAlong);
	...
	virtual unsigned int GetContainerSize() {return GetCount();}
	virtual unsigned int GetContainerUsed() {return GetCount();}
	virtual void OnRemoveType(class LSTypeDefinition *pType);
...
public:
	...
	inline unsigned int GetCount() {return Set.size();}
	virtual LSIterator *NewIterator(void *PassAlong);
...
public:
	...
	virtual bool SetItem(unsigned int N,int argc, char *argv[]);
	virtual bool DeleteItem(unsigned int N);
	virtual unsigned int GetNextItem(unsigned int N);

	virtual bool Resize(unsigned int NewSize);
	virtual bool Swap(unsigned int A, unsigned int B);
	virtual bool Move(unsigned int From, unsigned int To);

	virtual bool Clear();
	virtual void Collapse();
	bool ShiftUp(unsigned int From, unsigned int count);

	virtual PLSOBJECT GetItem(unsigned int N);
	...
	inline unsigned int GetAllocated()
	{
		return Index.Size;
	}
	...
	virtual unsigned int GetContainerSize() {return GetAllocated();}
	virtual unsigned int GetContainerUsed() {return GetUsed();}
	virtual LSIterator *NewIterator(void *PassAlong);
	virtual void OnRemoveType(class LSTypeDefinition *pType);
