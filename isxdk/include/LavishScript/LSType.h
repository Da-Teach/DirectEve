...
typedef bool (__cdecl *fLSTypeMember)(LSOBJECTDATA, int argc, char *argv[], LSOBJECT &);
typedef bool (__cdecl *fLSTypeMethod)(LSOBJECTDATA &, int argc, char *argv[]);
...
{
public:
	LSBinary(unsigned int InitialSize=64);
	LSBinary(void *buf, unsigned int buflen);
	~LSBinary();

	bool Copy(void *buf, unsigned int buflen);
	unsigned int Resize(unsigned int NewSize);
	bool Reset(unsigned char Value);
	bool Reset(unsigned int Begin, unsigned int Length, unsigned char Value);
	bool Copy(unsigned int Begin, void *buf, unsigned int buflen);
	...
	inline unsigned int GetUsed() { return Used; }

	inline void SetUsed(unsigned int NewUsed) {Used=NewUsed;}
...
#pragma warning( pop )
	...
	virtual void InitVariable(LSOBJECTDATA &ObjectData) {ObjectData.Ptr=0;}
...
public:
	...
	bool Initialize(LSTypeDefinition *pType, const char *SubType, int argc=0, char *argv[]=0)
	{
		...
		if (!pType)
			return false;
		if (!pType->InitVariable(ObjectData,SubType))
			return false;
		if (!pType->FromText(ObjectData,argc,argv))
		{
			pType->FreeVariable(ObjectData);
			return false;
		}
		Type=pType;
		return true;
